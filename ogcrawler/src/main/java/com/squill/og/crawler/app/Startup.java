...
import com.squill.og.crawler.internal.RssSiteImpl;
...
public class Startup {
	...
	public Startup(String[] args) {
		...
		options.addOption("f", "file", true,
				"Configuration File For Crawler Definition");
	}
	...
	private void startApp() throws BeansException, JAXBException {
		try {
			...
			if (command.hasOption("h")) {
				...
				System.setProperty(SystemPropertyKeys.WEB_CRAWLERS_CONFIG_FILE,
						optionValue);
				File file = new File(optionValue);
				System.setProperty(SystemPropertyKeys.WEB_CRAWLERS_CONFIG_DIR,
						file.getParent());
				File parentFile = file.getParentFile();
				System.setProperty(SystemPropertyKeys.WEB_CRAWLERS_BASE_DIR,
				...
				List<IWebCrawlable> websites = readCrawlableWebSites(crawlersDef);
				List<IWebCrawlable> webApis = readRegisteredWebApis(crawlersDef);
				List<IWebCrawlable> rssSites = readCrawlableRssSites(crawlersDef);
				List<ICrawlable> allCrawlables = new ArrayList<ICrawlable>();
				allCrawlables.addAll(websites);
				allCrawlables.addAll(webApis);
				allCrawlables.addAll(rssSites);
				/*
				 * if(allCrawlables.isEmpty()) return;
				 */
	...
	/*
	 * private void check() { try { IRssFeedService service =
	 * ServiceRegistry.INSTANCE .findCompositeService(IRssFeedService.class);
	 * if(service != null) { LOG.info("NOT NULL"); LOG.debug("NOT NULL"); }
	 * LOG.info("NULL"); LOG.debug("NULL"); } catch (PackPackException e) {
	...
	private WebCrawlers readCrawlerDefinition() throws JAXBException {
		String loc = System
				.getProperty(SystemPropertyKeys.WEB_CRAWLERS_CONFIG_FILE);
	...
	private List<IWebCrawlable> readCrawlableWebSites(WebCrawlers crawlersDef) {
		List<IWebCrawlable> webSites = new ArrayList<IWebCrawlable>();
		List<WebCrawler> crawlers = crawlersDef.getWebCrawler();
		for (WebCrawler crawler : crawlers) {
			IWebCrawlable webSite = new WebsiteImpl(crawler,
					crawlersDef.getWebLinkTracker(), crawler.getFeedHandler());
	...
	private List<IWebCrawlable> readRegisteredWebApis(WebCrawlers crawlersDef) {
		...
		for (ApiReader apiReader : apiReaders) {
			IWebCrawlable webApi = new WebApiImpl(apiReader,
					crawlersDef.getWebLinkTracker(), apiReader.getFeedHandler());
			webApis.add(webApi);
		}
		return webApis;
	}

	private List<IWebCrawlable> readCrawlableRssSites(WebCrawlers crawlersDef) {
		List<IWebCrawlable> rssSites = new ArrayList<IWebCrawlable>();
		List<RssFeedReader> rssFeedReaders = crawlersDef.getRssFeedReader();
		for (RssFeedReader rssFeedReader : rssFeedReaders) {
			IWebCrawlable rssSite = new RssSiteImpl(rssFeedReader,
					crawlersDef.getWebLinkTracker(),
					rssFeedReader.getFeedHandler());
