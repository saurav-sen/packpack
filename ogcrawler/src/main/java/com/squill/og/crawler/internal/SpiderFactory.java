...
public class SpiderFactory {
	...
	public Spider createNewSpiderInstance(ICrawlable crawlable,
			...
			IWebLinkTrackerService trackerService, IFeedHandler handler,
		...
		if (crawlable instanceof IWebSite) {
			...
			return new WebSiteSpider(webSite,
					...
					webSite.getTrackerService(), handler, session);
			...
			return new WebApiSpider(webApi, crawlSchedulePeriodicTimeInMillis,
					webApi.getTrackerService(), handler, session);
		} else if (crawlable instanceof IRssSite) {
			IRssSite rssSite = (IRssSite) crawlable;
			return new WebFeedSpider(rssSite,
					crawlSchedulePeriodicTimeInMillis,
					rssSite.getTrackerService(), handler, session);
		}
		return null;
	}
