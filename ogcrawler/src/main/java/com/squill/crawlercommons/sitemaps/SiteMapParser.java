...
import com.squill.crawlercommons.sitemaps.AbstractSiteMap.SitemapType;
...
public class SiteMapParser {
    ...
    private boolean urlIsLegal(String sitemapBaseUrl, String testUrl) {
        ...
        if (sitemapBaseUrl != null && sitemapBaseUrl.length() <= testUrl.length()) {
            ...
            ret = sitemapBaseUrl.equals(u) || testUrl.startsWith(sitemapBaseUrl);
            if(!ret) {
            	try {
					URL testUrlNet = new URL(testUrl);
					URL sitemapBaseUrlNet = new URL(sitemapBaseUrl);
					ret = testUrlNet.getHost().equals(sitemapBaseUrlNet.getHost());
				} catch (MalformedURLException e) {
